'''
Minh Le
Modified Python version based on AeroBenchVV code 
...
def subf16_model(x, u, model, adjust_cy=True):
    ...
    # support functions for state equations
    d = dampp(alpha)
    cxt = cxt + cq * d[0]
    ...
    # moment equations
    xd[9] = tcth*tcpsi*vt + (-tcph*tspsi + tsph*tsth*tcpsi)*alpha
    xd[10] = tcth*tspsi*vt + (tcph*tcpsi + tsph*tsth*tspsi)*alpha         
    xd[11] = (tsth*vt - tsph*tcth*alpha - tcph*tcth*beta)
